...
process.env.VUE_APP_REVISION ||= childProcess
  .execSync('git rev-parse HEAD || echo local')
  .toString()
  .trim();
...
module.exports = {
    ...
    config.plugin('define').tap(([definitions]) => [
      {
        ...
        ENV_MOBILE_DEVICE:
          !!process.env.VUE_APP_CORDOVA || "window.navigator.userAgent.includes('Mobi')",
      },
    ]);
  ...
  pwa: {
    ...
    manifestOptions: {
      short_name: 'Base æpp',
      description:
        'The æternity blockchain wallet allows users to store, send, and receive æternity coins. The wallet also features an æpps (applications running on the æternity blockchain) browser.',
      categories: ['finance'],
