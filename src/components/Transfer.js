...
export default {
  ...
  computed: {
    ...
    errors () {
      ...
      if (!web3.isAddress(this.addressFrom)) { errors.push('Address From is invalid') }
      if (!web3.isAddress(this.addressTo)) { errors.push('Address To is invalid') }
      if (web3.isAddress(this.addressFrom) && this.addressFrom === this.addressTo) { errors.push('Addresses must be different') }
      if (parseFloat(this.amount) <= 0.0) { errors.push('Amount must be greater than zero') }
      // if(address
      return errors
    },
    allInputValid () {
