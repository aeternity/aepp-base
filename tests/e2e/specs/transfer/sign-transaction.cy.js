...
describe('Sign transaction', () => {
  ...
  transactions.forEach((txParams) => {
    it(`shows ${Tag[txParams.tag]} details`, () => {
      const url = new URL('http://localhost/sign-transaction');
      url.searchParams.append('transaction', buildTx(txParams));
      url.searchParams.append('x-success', 'about:blank');
      cy
        .viewport('iphone-se2')
  ...
  it('signs transaction', () => {
    const url = new URL('http://localhost/sign-transaction');
    url.searchParams.append('transaction', buildTx(transactions[0]));
    url.searchParams.append('x-success', 'about:blank?transaction={transaction}');
    cy
      .viewport('iphone-se2')
      .visit(url.href.replace('http://localhost', ''), { login: true });
    ...
    cy.url().should('contain', 'about:blank').then((u) => {
      ...
      const signedTx = resultUrl.searchParams.get('transaction');
      const txParams = unpackTx(signedTx, Tag.SignedTx);
      expect(txParams.signatures).to.have.length(1);
    });
  });

  it('redirects to home page of cancelled without callback', () => {
    const url = new URL('http://localhost/sign-transaction');
    url.searchParams.append('transaction', buildTx(transactions[0]));
    url.searchParams.append('x-success', 'about:blank');
    cy
    ...
    cy.location('pathname').should('equal', '/transfer');
  });

  it('cancel transaction signing', () => {
    const url = new URL('http://localhost/sign-transaction');
    url.searchParams.append('transaction', buildTx(transactions[0]));
    url.searchParams.append('x-success', 'about:blank?success');
    url.searchParams.append('x-cancel', 'about:blank?cancel');
    cy
      .viewport('iphone-se2')
      .visit(url.href.replace('http://localhost', ''), { login: true });
    cy.get('button').contains('Cancel').click();
    cy.url().should('equal', 'about:blank?cancel');
  });
});
